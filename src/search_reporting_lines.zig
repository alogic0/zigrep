...
pub fn writeFileLines(
    ...
    if (try zigrep.search.io.decodeToUtf8Alloc(allocator, bytes, encoding)) |decoded| {
        ...
        if (searcher.program.can_match_newline) {
            if (invert_match or max_count != null) return error.InvalidFlagCombination;
            if (output.only_matching) {
                if (context_before != 0 or context_after != 0 or output_format == .json) return error.InvalidFlagCombination;
...
fn writeFileReportsReplacing(
    ...
    const haystack = if (try zigrep.search.io.decodeToUtf8Alloc(allocator, bytes, encoding)) |decoded| decoded else bytes;
    ...
    const Collector = struct {
        ...
        fn emit(self: *@This(), captured: zigrep.search.grep.CapturedMatchReport) !void {
            ...
            if (self.lines.items.len != 0 and self.lines.items[self.lines.items.len - 1].line_span.start == report.line_span.start) {
                ...
                current.segments[current.segments.len - 1] = .{ .match_span = report.match_span, .replacement = replacement };
                return;
            }
            ...
            segments[0] = .{ .match_span = report.match_span, .replacement = replacement };
    ...
    return .{ .matched = true, .matched_lines = lines.items.len, .matches = match_count };
}
...
fn writeFileReportsWithContext(
    ...
    const Collector = struct {
        ...
        fn emit(self: *@This(), report: zigrep.search.grep.MatchReport) !void {
            while (self.next_line_index < self.line_spans.len and self.line_spans[self.next_line_index].start != report.line_span.start) {
                self.next_line_index += 1;
            }
            if (self.next_line_index >= self.line_spans.len) return error.InvalidFlagCombination;
            try self.items.append(self.allocator, .{ .line_index = self.next_line_index, .report = report });
            if (self.max_count) |limit| {
    ...
    return .{ .matched = true, .matched_lines = matched_lines.items.len, .matches = matched_lines.items.len };
}
...
fn writeFileReportsWithContextReplacing(
    ...
    return .{ .matched = true, .matched_lines = matched_lines.items.len, .matches = match_count };
}
...
fn writeInvertedFileReports(
    ...
    return .{ .matched = selected_count != 0, .matched_lines = selected_count, .matches = selected_count };
}
...
fn expandReplacementAlloc(
    ...
    while (index < template.len) {
        ...
        if (next == '{') {
            ...
            try appendCaptureExpansion(allocator, &expanded, template[index + 2 .. close], match_span, groups, capture_names, line, line_span_start);
            index = close + 1;
            continue;
        }
        ...
        if (std.ascii.isDigit(next)) {
            ...
            try appendCaptureExpansion(allocator, &expanded, template[index + 1 .. end], match_span, groups, capture_names, line, line_span_start);
            index = end;
            continue;
        }
