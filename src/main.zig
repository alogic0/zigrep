...
pub fn runSearch(
    ...
    return runner.runSearch(allocator, stdout, stderr, options);
}
...
fn searchEntriesSequential(
    ...
    return runner.searchEntriesSequential(allocator, stdout, stderr, entries, options);
}
...
fn formatReport(
    ...
    return runner.formatReport(allocator, report, output);
}
...
fn reportFileMatch(
    ...
    return runner.reportFileMatch(allocator, searcher, path, bytes, encoding);
}
...
test "runCli supports basic class-set operators" {
    ...
    const subtraction_run = try runCliCaptured(testing.allocator, &.{ "zigrep", "[\\w--\\p{ASCII}]+", root_path });
    ...
    const intersection_run = try runCliCaptured(testing.allocator, &.{ "zigrep", "[\\p{Greek}&&\\p{Uppercase}]+", root_path });
...
test "runCli supports nested class-set expressions" {
    ...
    const run = try runCliCaptured(testing.allocator, &.{ "zigrep", "[\\w--[\\p{ASCII}&&[^_]]]+", root_path });
    defer run.deinit(testing.allocator);
    try testing.expectEqual(@as(u8, 0), run.exit_code);
    try testing.expect(!std.mem.containsAtLeast(u8, run.stdout, 1, "sample.txt:1:1:A"));
...
test "runCli supports unscoped inline flag toggles" {
    ...
    const local_case = try runCliCaptured(testing.allocator, &.{ "zigrep", "(?i)a(?-i)A", root_path });
    ...
    const ascii_word = try runCliCaptured(testing.allocator, &.{ "zigrep", "(?-u)\\w+", root_path });
    ...
    const local_dotall = try runCliCaptured(testing.allocator, &.{ "zigrep", "-U", "(?s)a.b", root_path });
